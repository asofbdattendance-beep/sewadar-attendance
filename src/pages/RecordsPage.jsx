...
function AttendanceTab() {
  ...
  async function fetchRecords() {
    ...
    const grouped = {}
    data?.forEach(r => {
      const dt = new Date(r.scan_time); const date = `${dt.getFullYear()}-${String(dt.getMonth()+1).padStart(2,'0')}-${String(dt.getDate()).padStart(2,'0')}`
    ...
    // Fetch flagged IDs — chunked to avoid large .in() queries
    const allIds = filteredRecords.flatMap(r => [r.in_id, r.out_id].filter(Boolean))
    if (allIds.length > 0) {
      const CHUNK = 200
      const allFlagged = []
      for (let i = 0; i < allIds.length; i += CHUNK) {
        const { data: flagData } = await supabase.from('queries')
          .select('attendance_id').in('attendance_id', allIds.slice(i, i + CHUNK)).neq('status', 'resolved')
        if (flagData) allFlagged.push(...flagData.map(f => f.attendance_id))
      }
      setFlaggedIds(new Set(allFlagged))
    } else {
      ...
      <div className="records-filters" style={{ marginBottom: '1rem' }}>
        <div className="search-box" style={{ flex: 1 }}>
          ...
          <input type="text" placeholder="Search badge or name…" value={searchTerm}
            onChange={e => { const v = e.target.value; setSearchTerm(v); clearTimeout(window._recDebounce); window._recDebounce = setTimeout(() => fetchRecords(), 350) }} />
        </div>
        ...
        <div className="records-table-wrap records-table-responsive">
          <table className="records-table">
            ...
            <tbody>
              ...
              {records.length === 0 && (
                <tr><td colSpan={isAdmin ? 9 : 8} style={{ textAlign: 'center', padding: '2.5rem', color: 'var(--text-muted)' }}>No records found</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function ReportsTab() {
  ...
  async function runCentreWiseReport() {
    ...
    const _s1 = new Date(dateFrom || `${yearFilter}-01-01`); _s1.setHours(0,0,0,0)
    const _e1 = new Date(dateTo || `${yearFilter}-12-31`); _e1.setHours(23,59,59,999)
    let q = supabase.from('attendance').select('centre, badge_number, type, scan_time').gte('scan_time', _s1.toISOString()).lte('scan_time', _e1.toISOString())
  ...
  async function runYearlySatsangReport() {
    ...
    const centreNames = await getCentreNames()
    const _s2 = new Date(`${yearFilter}-01-01`); _s2.setHours(0,0,0,0)
    const _e2 = new Date(`${yearFilter}-12-31`); _e2.setHours(23,59,59,999)
    let attQ = supabase.from('attendance').select('badge_number, sewadar_name, centre, department, scan_time, type').gte('scan_time', _s2.toISOString()).lte('scan_time', _e2.toISOString()).eq('type', 'IN')
  ...
  async function runSewadarCountReport() {
    ...
    const centreNames = await getCentreNames()
    const _s3 = new Date(dateFrom || `${yearFilter}-01-01`); _s3.setHours(0,0,0,0)
    const _e3 = new Date(dateTo || `${yearFilter}-12-31`); _e3.setHours(23,59,59,999)
    let q = supabase.from('attendance').select('badge_number, sewadar_name, centre, department, type, scan_time').gte('scan_time', _s3.toISOString()).lte('scan_time', _e3.toISOString())
    if (centreNames) q = q.in('centre', centreNames)
    const { data } = await q.limit(50000)
    if (!data) { setLoading(false); return }
    const map = {}
